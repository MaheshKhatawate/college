...
function Header() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 border-b border-amber-500/10 bg-gradient-to-b from-[#0b0b0d]/70 to-[#0f172a]/60 supports-[backdrop-filter]:backdrop-blur-xl">
            <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-4 flex items-center justify-between min-h-[48px] sm:min-h-[unset]">
                {/* Brand */}
                <button
                    onClick={() => navigate('/')}
                ...
                >
                    ...
                    <img
                        ...
                        alt=""
                        aria-hidden="true"
                        className="h-10 w-10 sm:h-18 sm:w-18 object-contain drop-shadow"
                        decoding="async"
                        draggable="false"
                    />
                    <h1 className="text-lg sm:text-2xl font-extrabold tracking-tight bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 bg-clip-text text-transparent group-hover:from-amber-300 group-hover:to-amber-500 transition-colors">
                        AyurDiet Pro
                    </h1>
                </button>
                ...
                <div className="relative w-full max-w-md hidden sm:block" role="tablist" aria-label="Primary">
                    <div className="relative p-1 rounded-full border border-amber-500/20 bg-white/[0.03] shadow-[inset_0_0_0_1px_rgba(255,191,0,0.06)]">
                        {/* Animated indicator: always rendered for smooth transition */}
                        <span
                            className={`absolute top-1 bottom-1 left-1 w-1/2 rounded-full bg-gradient-to-br from-amber-400/25 to-amber-300/10 border border-amber-400/40 shadow-[0_0_0_1px_rgba(255,193,7,0.15),0_8px_30px_rgba(255,193,7,0.08)] backdrop-blur-md transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]
                                ${hoveredTab === 'patient' ? 'translate-x-full' : 'translate-x-0'}
                                ${hoveredTab ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}
                            aria-hidden="true"
                        />
