    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={"px-3 py-1 cursor-pointer text-sm bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-2 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"}
            disabled={isFetchingPatients || refreshLoading}
          >
            {(isFetchingPatients || refreshLoading) && (
              <svg className="animate-spin h-4 w-4 mr-1 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
              </svg>
            )}
            {(isFetchingPatients || refreshLoading) ? 'Refreshing...' : 'Refresh'}
          </button>
          {message && (
      ...
      <div className="flex gap-2 text-sm">
        {['add', 'list', 'stats'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={cn("px-4 py-2 cursor-pointer rounded-md border transition", activeTab === tab ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/70")}>{tab === 'add' ? 'Add / Edit' : tab === 'list' ? 'Patient List' : 'Statistics'}</button>
        ))}
      </div>
